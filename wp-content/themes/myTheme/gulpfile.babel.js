...
gulp.task( 'allJS', () => {
	return gulp
		.src( config.jsSRC, { since: gulp.lastRun( 'allJS' ) }) // Only run on changed files.
		.pipe( plumber( errorHandler ) )
		// .pipe(
		// 	babel({
		// 		presets: [
		...
		// 	})
		// )
		.pipe( remember( config.jsSRC[0] ) ) // Bring all files back to stream.
		.pipe( remember( config.jsSRC[1] ) ) // Bring all files back to stream.
		.pipe( remember( config.jsSRC[2] ) ) // Bring all files back to stream.
...
gulp.task(
	'default',
	gulp.parallel( 'styles', 'allJS', 'images', browsersync, () => {
		gulp.watch( config.watchPhp, reload ); // Reload on PHP file changes.
		gulp.watch( config.watchStyles, gulp.parallel( 'styles' ) ); // Reload on SCSS file changes.
		gulp.watch( config.watchJs, gulp.series( 'allJS', reload ) ); // Reload on allJS file changes.
